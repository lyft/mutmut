...
def run_result_browser(
    *,
    ...
    function_dependencies: dict[str, set[str]] | None = None,
) -> None:
    ...
    Args:
        ...
        walk_source_files: Callable that returns a generator yielding source file paths.
        ...
        current_hashes: Current function hashes for cache status calculation.
        function_dependencies: Map of function -> functions it depends on.
    """
    ...
    class ResultBrowser(App[None]):
        ...
        # Depth mode index cycles through available options
        dep_level_index = 0
        ...
        columns: list[tuple[str, str | Text]] = [("path", "Path"), ("cache", Text("Cache", justify="right"))]
        for status, emoji in emoji_by_status.items():
            columns.append((status, Text(emoji, justify="right")))
        ...
        def on_mount(self) -> None:
            ...
            # Set up depth options: [1, config_depth, 0(full)] or [1, 0(full)] if config is -1/full
            configured_depth = config().get_effective_dependency_depth()
            if configured_depth == -1 or configured_depth == 0:
                # Config is already "full", so only two options: single and full
                self._depth_options: list[tuple[int, str]] = [(1, "1-lvl"), (0, "full")]
            elif configured_depth == 1:
                # Config is already "1", so only two options: single and full
                self._depth_options = [(1, "1-lvl"), (0, "full")]
            else:
                # Three options: single, configured, full
        ...
        def read_data(self) -> None:
            ...
            self.path_by_name = {}
            self.cached_by_path: dict[str, CacheStatus] = {}  # Aggregate cached status per file

            # Load dependency tracking stats if available
            self.deps_available = load_stats()

            # Recompute current function hashes from source files
            # This ensures we have fresh hashes after running generate/run
            ...
            # Collect functions with invalid mutants (either exit_code is None or hash changed)
            self.invalid_raw_funcs: set[str] = set()
            # Track which mutants are invalid due to hash change (for display purposes)
            ...
            for p in walk_source_files():
                ...
                for name, exit_code in source_file_mutation_data.exit_code_by_key.items():
                    ...
                    # Track functions with invalid mutants for dependency checking
                    if exit_code is None:
                        self.invalid_raw_funcs.add(raw_name)
                    elif fresh_hashes:
                        # Check if function hash has changed (source modified but not regenerated)
                        # Extract just the function name for hash lookup
            ...
            # Pre-compute all functions that depend on invalid functions to keep the ui responsive
            if self.deps_available:
                ...
                # Precompute raw_deps (mangled->raw name conversion) once for dependency display
            ...
            # Cache for dependency BFS results: (func_name, max_depth) -> (upstreams, downstreams)
            self._deps_cache: dict[tuple[str, int], tuple[list[tuple[str, int]], list[tuple[str, int]]]] = {}

            # Compute aggregate validity per file (worst status: INVALID > STALE_DEPENDENCY > CACHED)
            for path_str, (source_file_mutation_data, _) in self.source_file_mutation_data_and_stat_by_path.items():
                ...
                for name, exit_code in source_file_mutation_data.exit_code_by_key.items():
                    # Override exit_code to None for hash-changed mutants
                    effective_exit_code = None if name in self.hash_changed_mutants else exit_code
                    status = get_cache_status(name, effective_exit_code, self.funcs_with_invalid_deps)
                    worst = CacheStatus.worst(worst, status)
        ...
        def populate_files_table(self) -> None:
            ...
            for p, (_, stat) in sorted(self.source_file_mutation_data_and_stat_by_path.items()):
                ...
                row = (
                    [p]
                    + [CACHE_STATUS_EMOJI[cached_status]]  # cache column (index 1)
                    + [
            ...
            # Refresh mutants table for currently selected file
        ...
        def _refresh_mutants_table(self) -> None:
            ...
            try:
                ...
                if not row_key:
                    return
                path = row_key[0]  # First column is the path
            except Exception:
                return
            ...
            for k, v in source_file_mutation_data.exit_code_by_key.items():
                ...
                if status != "killed" or show_killed or validity != CacheStatus.CACHED:
        ...
        def action_retest_dependencies(self) -> None:
            ...
            This is useful when a mutant has ⚠️ status - it tests the underlying
            functions that have changed, which is what caused the warning.
            """
        ...
        def _update_dependencies_display(self, mutant_name: str) -> None:
            ...
            # Get depth from current mode (0 means unlimited)
