...
def get_ordered_upstream_and_downstream_functions(
    ...
    while up_queue:
        ...
        for caller in callers:
            # Skip the source function (avoid showing self in cycles)
    ...
    # Downstream: functions that raw_func_name calls (where raw_func_name is a caller)
    # raw_deps is callee -> set of callers, so we need to find callees where raw_func_name is in callers
    down_queue = [(raw_func_name, 0)]
    downstreams: dict[str, int] = {}

    while down_queue:
        func, depth = down_queue.pop()
        # Find all callees where func is a caller
...
def compute_funcs_with_invalid_deps(invalid_raw_funcs: set[str], deps: dict[str, set[str]]) -> set[str]:
    ...
    This is done once at load time to avoid expensive BFS per-row.
    ...
    # Convert deps to raw names for consistent lookup
    ...
    # Use expand_changed_functions to find all callers transitively,
    # but exclude the invalid functions themselves - we only want their callers
...
def find_invalid_dependencies(
    ...
    # Build callees graph (caller -> callees) with raw names
    ...
    # BFS to find all callees transitively
    visited: set[str] = set()
    queue = [raw_func_name]
