...
def write_all_mutants_to_file(
    *, out: io.TextIOWrapper, source: str, filename: Path
    ...
    mutated_code, mutant_names, hash_by_function_name, metadata_by_name = mutate_file_contents(
        source, get_covered_lines_for_file(filename_str, state()._covered_lines)
    )
...
# used to copy the global mutmut.config to subprocesses in Pool
# guard against RuntimeError when context is already set (e.g., in workers)
try:
