...
class MutantStatus(Enum):
    ...
    KILLED = auto()  # Test failed - mutant detected
    SURVIVED = auto()  # Tests passed - mutant not detected
    TIMEOUT = auto()  # Tests timed out
    ERROR = auto()  # Exception during test run
    NO_TESTS = auto()  # No tests cover this mutant
    SKIPPED = auto()  # Mutant was skipped
    SUSPICIOUS = auto()  # Unknown/unexpected exit code
    SEGFAULT = auto()  # Process crashed with segfault

    @classmethod
    def from_exit_code(cls, exit_code: int) -> "MutantStatus":
        """Convert an exit code to a MutantStatus.

        This mapping mirrors status_by_exit_code from __main__.py.

        Args:
            exit_code: The exit code from the test runner.

        Returns:
            The corresponding MutantStatus.
        """
        match exit_code:
            case 1 | 3 | -24:
                return cls.KILLED
...
class MutantRunner(ABC):
    ...
    mutants. Different implementations support different trade-offs:

    - ForkRunner: Fast (uses os.fork), but incompatible with some libraries
      (gevent, grpc, torch) that don't survive fork correctly.

    - HotForkRunner: Fork-safe runner that imports pytest in a orchestrator
      process, then forks per mutant. Nearly as fast as ForkRunner.
...
class HotForkRunner(MutantRunner):
    ...
    def _orchestrator_main(self, work_fd: int, result_fd: int) -> None:
        ...
        # Set up file logging for debugging (same as subprocess workers)
        setup_file_logging()
        ...
        while True:
            ...
            if child_pid == 0:
                ...
                status_name = MutantStatus.from_exit_code(exit_code).name
    ...
    def _reap_children(
        ...
        while True:
            ...
            status_name = MutantStatus.from_exit_code(exit_code).name
            orchestrator_logger.debug(
                f"Completed {child.mutant_name}: {status_name} (exit={exit_code}, {duration:.2f}s)"
            )
    ...
    def _check_orchestrator_alive(self) -> None:
        ...
        try:
            ...
            if pid == self.orchestrator_pid:
                ...
                # Non-zero exit code means crash - attempt restart
            ...
            # No child to wait for - orchestrator is gone
    ...
    def shutdown(self) -> None:
        ...
        # Close work pipe to signal orchestrator to finish (if not already closed)
