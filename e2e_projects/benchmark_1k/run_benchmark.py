...
def main():
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Show all mutmut output including debug")
    parser.add_argument(
        "--quiet", "-q", action="store_true", default=True, help="Filter output to only show progress (default: True)"
    )
    parser.add_argument(
        "--no-quiet", action="store_false", dest="quiet", help="Show all stdout from mutmut (including test names)"
    )
    parser.add_argument(
        "--test-delay",
        type=float,
