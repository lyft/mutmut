...
from mutmut.workers.isolation import MutantStatus
...
class TestMutantStatus:
    """Tests for MutantStatus enum."""

    def test_from_exit_code_killed(self):
        """Exit codes 1, 3, -24 map to KILLED."""
        assert MutantStatus.from_exit_code(1) == MutantStatus.KILLED
        assert MutantStatus.from_exit_code(3) == MutantStatus.KILLED
        assert MutantStatus.from_exit_code(-24) == MutantStatus.KILLED

    def test_from_exit_code_survived(self):
        """Exit code 0 maps to SURVIVED."""
        assert MutantStatus.from_exit_code(0) == MutantStatus.SURVIVED

    def test_from_exit_code_no_tests(self):
        """Exit codes 5, 33 map to NO_TESTS."""
        assert MutantStatus.from_exit_code(5) == MutantStatus.NO_TESTS
        assert MutantStatus.from_exit_code(33) == MutantStatus.NO_TESTS

    def test_from_exit_code_timeout(self):
        """Timeout exit codes map to TIMEOUT."""
        assert MutantStatus.from_exit_code(36) == MutantStatus.TIMEOUT
        assert MutantStatus.from_exit_code(24) == MutantStatus.TIMEOUT
        assert MutantStatus.from_exit_code(152) == MutantStatus.TIMEOUT
        assert MutantStatus.from_exit_code(255) == MutantStatus.TIMEOUT

    def test_from_exit_code_segfault(self):
        """Exit codes -11, -9 map to SEGFAULT."""
        assert MutantStatus.from_exit_code(-11) == MutantStatus.SEGFAULT
        assert MutantStatus.from_exit_code(-9) == MutantStatus.SEGFAULT

    def test_from_exit_code_suspicious(self):
        """Unknown exit codes map to SUSPICIOUS."""
        assert MutantStatus.from_exit_code(99) == MutantStatus.SUSPICIOUS
        assert MutantStatus.from_exit_code(-99) == MutantStatus.SUSPICIOUS
